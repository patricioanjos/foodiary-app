...
export function Button({ 
    ...
    size = 'default', color = 'default', ...props 
}: IButtonProps) {
    const isChildrenString = typeof children === 'string'

    const childEl = !isChildrenString ? children : (
        <Text className="text-base font-sans-regular">{children}</Text>
    )
    ...
    return (
        <View className={cn('rounded-xl overflow-hidden', className)}>
            <Pressable
                ...
                android_ripple={{color: color === 'dark' ? 'rgba(255, 255, 255, 0.2)' : 
                    'rgba(0, 0, 0, 0.2)'
                }}
